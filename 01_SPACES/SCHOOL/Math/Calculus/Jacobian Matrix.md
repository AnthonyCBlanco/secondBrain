### Idea
In single-variable calculus, the derivative $f'(x)$ gives the best linear approximation to a function at a point (the tangent line). 
The **Jacobian Matrix** is the exact multivariable generalization of the derivative. For a vector-valued function with multiple inputs and multiple outputs, the Jacobian matrix packages all of its first-order [[Partial Derivative|partial derivatives]] into a single matrix. It represents the best linear approximation to the function near a given point.
...
### Key Applications
1. **[[Multivariable Chain Rule]]**: Just as the 1D chain rule is $f'(g(x)) \cdot g'(x)$, the multivariable chain rule is simply the matrix multiplication of their respective Jacobian matrices: $J_{F \circ G} = J_F \cdot J_G$.
